...
import {canJoin} from "../lib/matchmaking-marketplace.ts";
...
export async function deliverMarketplaceNotifications() {
  ...
  if(!await isMarketplaceReady())return;
  ...
  await db.query(`INSERT INTO matchmaking_delivery(id,session_id,player_id,kind,revision)
    ...
    WHERE s.source<>'legacy' AND s.status IN ('playable','full','completed') AND s.end_at>now()-interval '1 day'
      AND s.start_at<now()+interval '1 hour' ON CONFLICT DO NOTHING`);
  const jobs=await db.transaction(async tx=>tx.query<{id:string;sessionId:string;playerId:string;kind:"invite"|"playable"|"reopened"|"recruit"|"reminder"|"result";revision:number}>(`WITH candidates AS (
  ...
  for(const job of jobs){
    ...
    const valid=s&&((job.kind==="invite"&&m?.status==="pending"&&Date.parse(s.endAt)>Date.now())
      ||(job.kind==="recruit"&&Date.parse(s.endAt)>Date.now()&&pool.slots.some(a=>a.playerId===job.playerId&&!a.cancelled&&a.commitment==="going"&&Date.parse(a.endAt)>Date.now()&&canJoin(a,s,pool)))
      ||(["playable","reminder","result"].includes(job.kind)&&m?.status==="accepted"&&["playable","full","completed"].includes(s.status))
      ||(job.kind==="reopened"&&m?.status==="accepted"&&s.status==="forming"));
    if(!valid){await db.query(`UPDATE matchmaking_delivery SET sent_at=now() WHERE id=$1`,[job.id]);continue;}
    ...
    if(outcome.sent)await db.query(`UPDATE matchmaking_delivery SET sent_at=now() WHERE id=$1`,[job.id]);
  }
}
