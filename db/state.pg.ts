...
function ensureStateSchema() {
  schemaReady ??= (async () => {
    ...
    await sql`ALTER TABLE state_matches ADD COLUMN IF NOT EXISTS player_a2 text`;
    await sql`ALTER TABLE state_matches ADD COLUMN IF NOT EXISTS player_b2 text`;
    await sql`ALTER TABLE state_matches ADD COLUMN IF NOT EXISTS mode text`;
    await sql`ALTER TABLE state_matches ADD COLUMN IF NOT EXISTS before_a2 numeric`;
    await sql`ALTER TABLE state_matches ADD COLUMN IF NOT EXISTS before_b2 numeric`;
    await sql`ALTER TABLE state_matches ADD COLUMN IF NOT EXISTS after_a2 numeric`;
    await sql`ALTER TABLE state_matches ADD COLUMN IF NOT EXISTS after_b2 numeric`;
