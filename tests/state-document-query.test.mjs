...
test("the single-statement document matches the five-query document exactly", { skip: url ? false : "set TEST_DATABASE_URL" }, async () => {
  ...
  await sql.unsafe(`
    ...
    CREATE TABLE state_tournaments (id text PRIMARY KEY, name text NOT NULL, handicap_mode text NOT NULL, start_at timestamptz, signup_deadline timestamptz NOT NULL, created_at timestamptz NOT NULL, created_by text, signups jsonb DEFAULT '[]'::jsonb NOT NULL, draw jsonb, drawn_at timestamptz, walkovers jsonb);
  ...
  await sql.unsafe(`
    ...
    INSERT INTO state_tournaments VALUES ('t1','冬季盃','suggested','2026-01-25T15:00:00Z','2026-01-20T15:00:00Z','2026-01-10T00:00:00Z','p1','["p1","p2"]','["p1","p2"]','2026-01-21T02:00:00.123Z','[{"round":1,"index":0,"winner":"p1"}]');
    INSERT INTO state_tournaments VALUES ('t2','夏季盃','none','2026-06-25T15:00:00Z','2026-06-20T15:00:00Z','2026-06-01T00:00:00Z',NULL,'[]',NULL,NULL,NULL);
  ...
  const [players, matches, tournaments, settings, audits] = await Promise.all([
    ...
    sql`SELECT id, name, handicap_mode AS "handicapMode", to_char(start_at AT TIME ZONE 'Asia/Hong_Kong', 'YYYY-MM-DD"T"HH24:MI') AS "startAt", to_char(signup_deadline AT TIME ZONE 'Asia/Hong_Kong', 'YYYY-MM-DD"T"HH24:MI') AS "signupDeadline", created_at AS "createdAt", created_by AS "createdBy", signups, draw, drawn_at AS "drawnAt", walkovers FROM state_tournaments ORDER BY created_at DESC`,
