...
const questionOne = [
  ["450", "打唔中白波／白波打唔中目標波"], ["550", "大部分情況都可以打中目標波"],
  ["1000", "可以穩定打入一個波"], ["1200", "可以穩定連續打入一組（兩個）波"],
  ["1500", "可以打到多杆（15+）"], ["1700", "可以打到多杆（30+）"], ["1900", "可以打到多杆（50+）"],
  ["2100", "可以打到多杆（70+）"], ["2400", "可以打到多杆（90+）"],
  ["2800", "一生人打過 1 杆百位（Century）"], ["3300", "打過多於 1 杆百位（Centuries）"],
] as const;
...
function RatingStep({ displayName, onBack, onDone }: { displayName: string; onBack: () => void; onDone: (rating: number) => void }) {
  ...
  return <>
    <h1>了解你嘅打波水平</h1>
    <p className="onboarding-intro">{displayName}，答一條問題，幫你設定一個更貼近實力嘅初始評級。</p>
    ...
    <div className="onboarding-actions">
      ...
      <Button className="onboarding-submit" onClick={submit} disabled={submitting}>{submitting ? "儲存中…" : "提交"}</Button>
    </div>
  </>;
}
