...
export function RecentMatches({points,onViewAll,onMatch}:{points:EloTrendPoint[];onViewAll?:()=>void;onMatch:(matchId:string)=>void}) {
  ...
  return <section className="profile-section recent-form" aria-labelledby="recent-form-title"><div className="profile-section-head"><div><p className="kicker">近期狀態</p><h3 id="recent-form-title">最近五場</h3></div>{onViewAll&&<button type="button" className="recent-form-action" onClick={onViewAll}><span>查看所有賽事</span><i aria-hidden="true">→</i></button>}</div>{matches.length===0?<p className="recent-form-empty">尚未有比賽記錄</p>:<div className="recent-match-grid">{matches.map(point=><button type="button" className={`recent-result ${point.result.toLowerCase()}`} key={point.id} onClick={()=>onMatch(point.id)} aria-label={`查看對 ${point.opponent} 的賽事：${point.score}`}><div><b>{point.result==="W"?"勝":point.result==="L"?"負":"和"}</b><time>{point.date.slice(5).replace("-","/")}</time></div><strong>{point.score}</strong><span><i>{point.opponentShort}</i>{point.opponent}</span><small className={point.delta>=0?"positive":"negative"}>{point.delta>=0?"+":""}{Math.round(point.delta)} ELO</small></button>)}</div>}</section>;
}
