...
function Leaderboard({ranked,data,onRecord,onPlayer}:{ranked:Player[];data:AppState;onRecord:()=>void;onPlayer:(p:Player)=>void}) {
  ...
  return <><section className="hero"><div><p className="kicker">SCAA 公開排名</p><h1>排行榜</h1><p>每一場，都清楚計算。</p></div><button className="primary" onClick={onRecord}>＋ 記錄新賽果</button></section>
    ...
    <div className="table-card">{ranked.length===0?<Empty text="尚未有球員" sub="前往球員頁面新增第一位球員。"/>:<><div className="table-head"><span>排名</span><span>球員</span><span><Term label="近況" tip="最近五筆比賽紀錄：W 代表勝、L 代表負、D 代表和。"/></span><span>場數／<Term label="勝率" tip="勝出場數除以全部已記錄比賽場數；和局亦計入總場數。"/></span><span><Term label="正式評分" tip="由球會職員在現實中維護的讓分評分，只作參考，不會由 ELO 自動修改。"/>／<Term label="建議評分" tip="系統根據球員相對 ELO 表現換算出的建議讓分評分。"/></span><span><Term label="ELO" tip="反映球員相對實力的動態評分；賽果會按對手、讓分、勝負及局數更新。"/></span></div>
