...
type AccountMember = {
  ...
  statePlayerId?: string; avatar?: string | null; initials?: string | null; iconColour?: string | null;
  googleLinked?: boolean; hasPassword?: boolean;
};
type Player = {
...
function trendPoints(player: Player, matches: Match[], players: Player[]): EloTrendPoint[] {
  ...
  return [start, ...matches.map(match => {
    ...
    return {
      ...
      opponent, opponentShort: opponent, score: `${ownScore}–${opponentScore}`,
...
function matchRecords(player: Player, matches: Match[], players: Player[]): MatchRecord[] {
  return matches.map(match => {
    ...
    return {
      ...
      result: ownScore === opponentScore ? "D" : ownScore > opponentScore ? "W" : "L", score: `${ownScore}–${opponentScore}`, ownScore, opponentScore,
...
function AccountMetrics({ player, players, state }: { player: Player; players: Player[]; state: State }) {
  ...
  return <div className="account-hero-metrics">
