...
function MatchForm({data,draft,setDraft,preview,a,b,editing,onSave}:{data:AppState;draft:any;setDraft:any;preview:any;a:Player;b:Player;editing:boolean;onSave:()=>void}) {
  ...
  const breakCount=(draft.highBreaks??[]).length;
  ...
  return <div className="match-form"><div className="match-form-head"><p className="kicker">{editing?"修正賽事":"快速記錄"}</p><div className="match-title-row"><h2>{editing?"編輯比賽":"記錄比賽"}</h2><div className="match-date-chip"><button type="button" aria-label={`更改比賽日期，目前為${dateLabel}`} onClick={openDatePicker}><span>{dateLabel}</span></button><input ref={dateInputRef} tabIndex={-1} aria-label="比賽日期" type="date" value={draft.date} onChange={e=>update("date",e.target.value)}/></div></div></div>
    {editing&&<p className="sub">儲存後會按日期重播全部賽事，重建雙方及後續 ELO。</p>}
    {data.players.length<2&&<p className="warning">請先新增至少兩位活躍球員。</p>}
    <section className="match-players" aria-labelledby="match-players-title"><h3 id="match-players-title">選擇球員</h3><div><label><span>球員 A</span><select value={draft.a} onChange={e=>update("a",e.target.value)}>{players.map(p=><option value={p.id} key={p.id}>{p.name}</option>)}</select><small>{a?`${Math.round(a.rating)} ELO`:"—"}</small></label><i aria-hidden="true">對</i><label><span>球員 B</span><select value={draft.b} onChange={e=>update("b",e.target.value)}>{players.map(p=><option value={p.id} key={p.id}>{p.name}</option>)}</select><small>{b?`${Math.round(b.rating)} ELO`:"—"}</small></label></div></section>
    <section className="quick-handicap" aria-labelledby="handicap-title"><div><h3 id="handicap-title">讓分</h3><small>{handicapLabel}</small></div><div className="handicap-actions"><button type="button" className={!draft.giver&&!customHandicap?"active":""} onClick={setNoHandicap}><span>沒有讓分</span><small>雙方平打</small></button><button type="button" disabled={fairActual==null} className={draft.giver&&+draft.points===fairPoints&&!customHandicap?"active":""} onClick={fairPoints===0?setNoHandicap:applyFair}><span>ELO 建議</span><small>{fairActual==null?"選擇球員":fairPoints===0?"毋須讓分":`${fairActual>=0?a?.name:b?.name} 讓 ${fairPoints} 分`}</small></button><button type="button" className={customHandicap?"active":""} onClick={()=>setCustomHandicap(value=>!value)}><span>自訂讓分</span><small>{customHandicap?"收起設定":"手動設定"}</small></button></div>
    ...
    <section className="score-panel" aria-labelledby="score-title">{preview&&<div className="predicted-ratio"><div><span>預測局數比例</span><b>{a.short} {Math.round(preview.expectedA*100)}% · {Math.round((1-preview.expectedA)*100)}% {b.short}</b></div><em aria-label={`${a.name} ${Math.round(preview.expectedA*100)}%，${b.name} ${Math.round((1-preview.expectedA)*100)}%`}><i style={{width:`${Math.round(preview.expectedA*100)}%`}}/></em></div>}<h3 id="score-title">最終比分</h3><div className="scoreboard-entry">
      <div><b>{a?.name??"球員 A"}</b><input aria-label={`${a?.name??"球員 A"}局數`} type="number" inputMode="numeric" min="0" value={draft.scoreA} onChange={e=>update("scoreA",Math.max(0,+e.target.value))}/><span><button type="button" aria-label={`${a?.name??"球員 A"}減一局`} onClick={()=>changeScore("scoreA",-1)}>−</button><button type="button" aria-label={`${a?.name??"球員 A"}加一局`} onClick={()=>changeScore("scoreA",1)}>＋</button></span></div>
      <strong aria-hidden="true">–</strong>
      <div><b>{b?.name??"球員 B"}</b><input aria-label={`${b?.name??"球員 B"}局數`} type="number" inputMode="numeric" min="0" value={draft.scoreB} onChange={e=>update("scoreB",Math.max(0,+e.target.value))}/><span><button type="button" aria-label={`${b?.name??"球員 B"}減一局`} onClick={()=>changeScore("scoreB",-1)}>−</button><button type="button" aria-label={`${b?.name??"球員 B"}加一局`} onClick={()=>changeScore("scoreB",1)}>＋</button></span></div>
    </div></section>
    <details className="match-optional"><summary><span>加入單桿記錄</span><em>{breakCount?`${breakCount} 筆`:"選填"}</em></summary><div className="break-entry">{[a,b].filter(Boolean).map(player=><section key={player.id}><div className="break-player"><i>{player.short}</i><b>{player.name}</b></div><div className="break-add"><input aria-label={`${player.name} 單桿分數`} type="number" inputMode="numeric" min="1" max="147" placeholder="分數" value={breakInput[player.id]??""} onChange={event=>setBreakInput(current=>({...current,[player.id]:event.target.value}))} onKeyDown={event=>{if(event.key==="Enter"){event.preventDefault();addBreak(player.id)}}}/><button type="button" onClick={()=>addBreak(player.id)}>加入</button></div><div className="break-chips">{(draft.highBreaks??[]).map((item:{playerId:string;value:number},index:number)=>item.playerId===player.id?<button type="button" key={index} onClick={()=>removeBreak(index)} aria-label={`移除 ${player.name} 的 ${item.value} 分單桿`}>{item.value}<span>×</span></button>:null)}</div></section>)}</div></details>
    {preview&&totalFrames>0&&<section className="elo-preview"><div><span><small>{a.name}</small><b className={preview.deltaA>=0?"positive":"negative"}>{preview.deltaA>=0?"+":""}{Math.round(preview.deltaA)} ELO</b></span><i aria-hidden="true">↔</i><span><small>{b.name}</small><b className={preview.deltaA<=0?"positive":"negative"}>{-preview.deltaA>=0?"+":""}{Math.round(-preview.deltaA)} ELO</b></span></div><details><summary>查看計算詳情</summary><p>{probabilities?`A 勝 ${Math.round(probabilities.win*100)}% · 和 ${Math.round(probabilities.draw*100)}% · `:""}表現分 {Math.round(preview.performanceScore*100)}% · 證據權重 ×{preview.evidenceWeight.toFixed(2)} · 讓分等效 {preview.adjustment>=0?"+":""}{Math.round(preview.adjustment)} ELO</p></details></section>}
