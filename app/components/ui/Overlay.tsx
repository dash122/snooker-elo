...
export function Sheet({open,title,children,onClose,className=""}:{open:boolean;title:string;children:ReactNode;onClose:()=>void;className?:string}){if(!open)return null;return <div className="ds-overlay ds-overlay--sheet" onMouseDown={event=>event.target===event.currentTarget&&onClose()}><section className={`ds-sheet${className?` ${className}`:""}`} role="dialog" aria-modal="true" aria-labelledby="ds-sheet-title"><button className="ds-dialog-close" onClick={onClose} aria-label="關閉">×</button><h2 id="ds-sheet-title">{title}</h2>{children}</section></div>}
