  ...
  return <main className="elo-guide" lang="en">
    ...
    <section className="guide-hero" id="top">
      ...
      <div className="hero-brief" aria-label="The model in brief">
        <div className="brief-topline"><span>Decision memo</span><span>Model v7</span></div>
    ...
    <section className="mechanism guide-section" id="mechanism">
      ...
      <ol className="process-list">
        ...
        <li><span className="process-index">2</span><div><h3>Estimate frame share</h3><p>Ratings and the applied handicap create a logistic forecast. The default scale is 500 ELO points, so a 50-point gap matters—but does not decide the match.</p></div><code>P<sub>A</sub> = 1 / (1 + 10<sup>−G/500</sup>)</code></li>
        ...
        <li><span className="process-index">4</span><div><h3>Price the evidence</h3><p>A logarithmic match-length scale rewards more evidence. A hyperbolic tangent then compresses extreme margins, so a blowout does not create an unlimited rating swing.</p></div><code>performance = S(n) × tanh(gap / w)</code></li>
    ...
    <section className="formula-section guide-section" aria-labelledby="formula-title">
      ...
      <div className="formula-card"><div className="formula-main">ΔA = <span>S(n)</span> × tanh <span className="formula-bracket">(</span><span className="fraction"><span>actual frames − expected frames</span><span className="fraction-line">width</span></span><span className="formula-bracket">)</span> × repetition factor</div><div className="formula-definitions"><span><b>S(n)</b> = 150 × ln((n + 15) / 10)</span><span><b>width</b> = 3 × 10<sup>−0.1/n</sup></span><span><b>repetition factor</b> = 2<sup>−t/7</sup></span></div></div>
    </section>
    ...
    <section className="worked-example guide-section" aria-labelledby="example-title">
      ...
      <div className="example-ledger"><div className="ledger-head"><span>Model input</span><span>Value</span><span>Why it matters</span></div><div className="ledger-row"><span>Expected frames for A</span><b>{workedExample.expectedFramesA.toFixed(2)}</b><small>Forecast before the match.</small></div><div className="ledger-row"><span>Actual frames for A</span><b>5.00</b><small>Observed performance.</small></div><div className="ledger-row"><span>Match-length scale</span><b>{workedExample.scale.toFixed(2)}</b><small>Six frames of evidence.</small></div><div className="ledger-row"><span>Repetition factor</span><b>{workedExample.repetitionFactor.toFixed(3)}</b><small>Three prior meetings in 30 days.</small></div><div className="ledger-row ledger-total"><span>Rating change for A</span><b>{signed(workedExample.deltaA, 2)} ELO</b><small>Player B moves by the opposite amount.</small></div></div>
    </section>
    ...
    <section className="guardrails guide-section" aria-labelledby="guardrails-title">
      ...
      <div className="guardrails-list"><article><span>Social mode</span><h3>2v2 entertainment matches are rating-neutral.</h3><p>The app records the score and the teams, but preserves every player’s official ELO, form, and record.</p></article><article><span>Audit trail</span><h3>Editing a match replays the history.</h3><p>Confirmed matches are rebuilt from each player’s initial rating in chronological order, so one corrected result can move later ratings too.</p></article><article><span>Early signal</span><h3>The first ten games stay provisional.</h3><p>New players are labelled provisional until they have enough observed evidence to read as a stable public number.</p></article><article><span>Fairness layer</span><h3>Handicap is a forecast adjustment, not a new rating.</h3><p>The default conversion is 25 ELO per handicap point, with 70% effectiveness in the frame forecast. The suggested club handicap is rounded from that same rating gap.</p></article></div>
    </section>
